...
bool P_Move (AActor *actor)
{
	...
	if (!try_ok)
	{
		...
		if ( NETWORK_GetState( ) == NETSTATE_SERVER )
		{
			SERVERCOMMANDS_MoveThing( actor, CM_XY|CM_Z|CM_MOVEDIR );
		}
...
void P_RandomChaseDir (AActor *actor)
{
	...
	if (pr_newchasedir() < 150)
	{
		if (P_TryWalk (actor))
		{
			...
			if ( NETWORK_GetState( ) == NETSTATE_SERVER )
				SERVERCOMMANDS_MoveThing( actor, CM_XY|CM_Z );
...
void A_DoChase (AActor *actor, bool fastchase, FState *meleestate, FState *missilestate, bool playactive, bool nightmarefast, bool dontmove)
{
		...
		NETWORK_ClientsideFunctionsAllowedOrIsServer( actor ))
	{
		...
		else
		{
			...
			if ( NETWORK_GetState( ) == NETSTATE_SERVER )
			{
				SERVERCOMMANDS_MoveThing( actor, CM_XY|CM_Z|CM_VELXY );
			}
		}
...
void P_TossItem (AActor *item)
{
	...
	if (style==2)
	{
		...
		if ( NETWORK_GetState() == NETSTATE_SERVER )
			SERVERCOMMANDS_MoveThing( item, CM_VELXY, MAXPLAYERS );
	}
	else
	{
		...
		if ( NETWORK_GetState() == NETSTATE_SERVER )
			SERVERCOMMANDS_MoveThing( item, CM_VELXY|CM_VELZ, MAXPLAYERS );
	}
}
