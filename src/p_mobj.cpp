...
//============================================================================
//
// AActor :: Get3dSectorBelow
//
//============================================================================

sector_t *AActor::GetSectorBelow(int flags) const
{
#ifdef _3DFLOORS
...
static void PlayerLandedOnThing (AActor *mo, AActor *onmobj)
{
	...
	else if (mo->velz < -FRACUNIT && mo->waterlevel < 2 && mo->player->mo && !mo->player->mo->isCrouchSliding)
	{
...
foundone:
	...
	if (smallsplash && splash->SmallSplash)
	{
		...
		if ( mo && ( NETWORK_GetState( ) == NETSTATE_SERVER ) )
		{
			...
			{
				int activatorPlayer = NETWORK_GetActorsOwnerPlayer( thing ) - players;
		...
		if (splash->SplashChunk)
		{
			...
			if ( NETWORK_GetState( ) == NETSTATE_SERVER )
			{
				if ( zacompatflags & ZACOMPATF_PREDICT_FUNCTIONS )
				{
					...
					SERVERCOMMANDS_SpawnThing( mo, activatorPlayer, SVCF_SKIPTHISCLIENT );
					SERVERCOMMANDS_MoveThing( mo, CM_VELXY|CM_VELZ, activatorPlayer, SVCF_SKIPTHISCLIENT );
				}
				else
				{
					SERVERCOMMANDS_SpawnThing( mo );
	...
	if (z != ONFLOORZ && z != ONCEILINGZ)
	{
		...
		if (source->player != NULL)	// Considering this is for player missiles, it better not be NULL.
		{
			...
			if (source->player->mo->AttackZOffset >= 0) {
				z += FixedMul(source->player->mo->AttackZOffset, source->player->crouchfactor);
			}
			else
			{
				z += FixedMul(source->player->mo->ViewHeight, source->player->crouchfactor) - (source->height >> 1) + FixedMul(source->player->mo->AttackZOffset, FRACUNIT);
			}
...
fixed_t AActor::GetGravity() const
{
	...
	return fixed_t(level.gravity * GetSectorBelow( FF_EXISTS )->gravity * FIXED2FLOAT(gravity) * 81.92);
}
